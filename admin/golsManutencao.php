...
if ($_POST) {
	...
	$id = save($data, "gols", $id);

	if(!$continue) header("Location: {$url}gols"); else header("Location: {$url}$page/Alterar/$id");
}else{
	if ($acao == "Alterar") {
		$sql = "SELECT * FROM gols WHERE id = $id";
...
<div class="center">
	<div id="tabs">
		...
		<?php else: ?>
			<form name="form1" id="form1" method="post" action="<?php echo $url; ?>golsManutencao/<?php echo $acao ?>/<?php echo $id ?>" onsubmit="lockButton(this);">
				<input type="hidden" name="acao" value="<?php echo $acao ?>" id="acao"/>
				<input type="hidden" name="id[]" value="<?php echo $id ?>" id="id[]" />
				<div class="sidebar left">
					<h2>Gols E.C União Santa Luiza &gt; <?php echo $acao; ?></h2>
					<ul>
				...
				<div class="contentManutencao right">
					<div class="t-prod" id="menu">
						<h2>Gols E.C União Santa Luiza &gt; <?php echo $acao; ?></h2>
						<input type="button" onclick="location.href='<?php echo $url; ?>gols'" class="btn-padrao voltar" id="voltar" value="Voltar" />
						<?php if($acao=="Alterar"): ?><input name="excluir" type="button" id="excluir" value="Excluir" class="btn-padrao excluir" onclick="deleteSelected(<?php echo $id ?>, 'gols', '<?php echo $url; ?>gols', true);" /><?php endif; ?>
							...
							<fieldset>
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr>
										<td width="180">
											<label>ID:</label>
											<label for="gols">Total de Gols: <span class="req">*</span></label>
										</td>
										<td>
											<input type="text" name="id" id="golsmanutencao" value="<?php echo $id ?>" readonly="readonly" />
											<input type="number" name="gols" id="golsmanutencao" value="<?php echo $gols ?>" />
										</td>
									</tr>
								</table>
							</fieldset>
